#!/usr/bin/env python3
"""
Vani Railway Track & Store Management Local Web Portal
Powered by Starlette and Uvicorn.
"""
...
async def search(request):
    ...
    for b in db.get('bridges', []):
        text = f"{b.get('bridge_no')} {b.get('section')} {b.get('bridge_type')} {b.get('old_bridge_no')} {b.get('km_from')} {b.get('km_to')}".lower()
        if q in text:
            results.append({
                'type': 'Bridge',
...
async def refresh_database(request):
    try:
        cmd = ["python3", str(BASE_DIR / 'indexer.py')]
        proc = subprocess.run(cmd, capture_output=True, text=True, check=True)
        global DB
        DB = load_db()
        return JSONResponse({'status': 'success', 'summary': DB.get('summary', {})})
...
async def media_handler(request):
    ...
    # Use FileResponse with inline disposition for PDFs
    return FileResponse(
        str(filepath),
...
routes = [
    ...
    Route('/api/refresh', refresh_database, methods=['POST']),
...
if __name__ == '__main__':
    ...
    print(f"Starting Vani Portal on http://localhost:{port}...")
